...
# Creates: Alembic config, initial migration, MongoDB indexes, seed data
...
echo "🔧 [11] Setting up database layer..."

# ═══════════════════════════════════════════════════════════════
# PART 1: ALEMBIC CONFIG
# ═══════════════════════════════════════════════════════════════

cat > "$CRUD/alembic.ini" << 'EOF'
[alembic]
script_location = alembic
prepend_sys_path = .
sqlalchemy.url = postgresql+asyncpg://afda:afda_secret@localhost:5432/afda_db
...
cat > "$CRUD/alembic/script.py.mako" << 'EOF'
"""${message}
...
cat > "$CRUD/scripts/seed_data.py" << 'PYEOF'
"""
...
# --- Create tables directly (no migration) ---
cat > "$CRUD/scripts/create_tables.py" << 'PYEOF'
"""
...
# --- Drop all tables ---
cat > "$CRUD/scripts/drop_tables.py" << 'PYEOF'
"""
...
# --- scripts/__init__.py ---
cat > "$CRUD/scripts/__init__.py" << 'EOF'
EOF

echo "✅ [11] Database layer created"
echo ""
echo "   $CRUD/"
echo "   ├── alembic.ini              — Alembic configuration"
echo "   ├── alembic/"
echo "   │   ├── env.py               — Async migration env (imports all models)"
echo "   │   ├── script.py.mako       — Migration template"
echo "   │   └── versions/            — Migration files go here"
echo "   └── scripts/"
echo "       ├── create_tables.py     — Create all tables directly"
echo "       ├── drop_tables.py       — Drop all tables"
echo "       └── seed_data.py         — Full seed: 6 KPIs, 4 actions, briefing,"
echo "                                  budget+line items, 5 variances, 3 forecasts,"
echo "                                  4 bank accounts, 30-day cash history,"
echo "                                  5 AR invoices, liquidity metrics, 10 COA,"
echo "                                  close period+6 tasks, 4 alerts, 3 rules,"
echo "                                  5 risk scores, 8 services, 3 users, 3 roles,"
echo "                                  6 settings, Mongo conversations+workflows"
echo ""
echo "   Usage:"
echo "   cd Services/afda-crud-api"
echo "   python -m scripts.create_tables   # create all tables"
echo "   python -m scripts.seed_data       # populate sample data"
echo ""
echo "   Or with Alembic:"
echo "   alembic revision --autogenerate -m 'initial'  # generate migration"
echo "   alembic upgrade head                           # apply migration"
echo ""
echo "   🎉 BACKEND IS FULLY SCAFFOLDED!"
echo "   Scripts 01-11 complete. Ready to run."
